...
'''
from rigid_matrix_vector import Rigid3Array
...
class InvariantPointAttentionMultimer(nn.Module):
    ...
    def forward(
        ...
        # self.linear_v = self.linear_v.to(device_now)
        # [*, N_res, H * C_hidden]
        v = self.linear_v(s)
        ...
        if h is not None:
            qimg = s @ h[...,0].view(s.shape[1],-1)
            qimg = qimg * scalar_weights
            kimg = s @ h[...,1].view(s.shape[1],-1)
            aimg = torch.einsum('...qhc,...khc->...qkh', qimg.view(a.shape[0],self.no_heads,-1), kimg.view(a.shape[0],self.no_heads,-1))
            # aimg = torch.einsum('...qhc,...khc->...qkh', q, kimg.view(a.shape[0],self.no_heads,-1))
            square_mask = mask.unsqueeze(-1) * mask.unsqueeze(-2)
            square_mask = self.inf * (square_mask - 1)
            aimg = aimg + square_mask.unsqueeze(-1)
            aimg = aimg * math.sqrt(1. / 3)  # Normalize by number of logit terms (3)
            aimg = self.softmax(aimg)
            vimg = s @ h[...,2].view(s.shape[1],-1)
            oimg = torch.einsum('...qkh, ...khc->...qhc', aimg, vimg.view(a.shape[0],self.no_heads,-1))
            # print('o',torch.max(o),torch.min(o),torch.mean(o),torch.std(o))

            oimg_mean = oimg.mean(dim=-1, keepdim=True)
            oimg_std = oimg.std(dim=-1, keepdim=True)
            oimg = (oimg - oimg_mean) / (oimg_std + 1e-6)

            o_mean = o.mean(dim=-1, keepdim=True)
            o_std = o.std(dim=-1, keepdim=True)

            oimg = oimg * o_std + o_mean
        ...
        # [*, N_res, H, P_v, 3]
        # self.linear_v_points = self.linear_v_points.to(device_now)
...
class StructureModule(nn.Module):
    ...
    def _forward_multimer_wrapper(self,
            evoformer_output_dict,
            aatype,
            mask=None,
            h=None,
        ...
        return self._forward_multimer(evoformer_output_dict, aatype, mask,h,inplace_safe, _offload_inference)
    ...
    def forward(
        ...
        """
        if(self.is_multimer):
            #outputs = self._forward_multimer(evoformer_output_dict, aatype, mask,h,inplace_safe, _offload_inference)
            outputs = checkpoint(self._forward_multimer_wrapper,evoformer_output_dict, aatype, mask,h,inplace_safe, _offload_inference,use_reentrant=False)
        else:
            outputs = self._forward_monomer(evoformer_output_dict, aatype, mask,h, inplace_safe, _offload_inference)
    ...
    def torsion_angles_to_frames(self, r, alpha, f):
        ...
        return torsion_angles_to_frames(r, alpha, f.to(alpha.device), self.default_frames.to(alpha.device))
    ...
    def frames_and_literature_positions_to_atom14_pos(
        ...
        return frames_and_literature_positions_to_atom14_pos(
            r,
            f.to(r.device),
            self.default_frames.to(r.device),
            self.group_idx.to(r.device),
            self.atom_mask.to(r.device),
            self.lit_positions.to(r.device),
        )
