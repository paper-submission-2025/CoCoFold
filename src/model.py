...
from utils import analyze_storage_size, deep_clone
...
class AlphaFold(nn.Module):
    ...
    def iteration(self, feats, prevs, _recycle=True, model_dir = None):
        ...
        if model_dir is not None:
            structure_path = model_dir+"structure.pth"
            print('save structure model data is true')
            print('structure_path',structure_path)
            save_data = {
                        'outputs':outputs,
                        'model_state':self.structure_module.state_dict(),
                        'feats':feats,
                        'type':s.dtype,
                        'config':self.config,
                        'm': m,
                        'x_prev': x_prev,
                        'inplace_safe':inplace_safe,
                        'offload_inference':self.globals.offload_inference,
                        'seq_mask':seq_mask,
                        'init_params': {
                            'c_s': self.structure_module.c_s,
                            'c_z': self.structure_module.c_z,
                            'c_ipa': self.structure_module.c_ipa,
                            'c_resnet': self.structure_module.c_resnet,
                            'no_heads_ipa': self.structure_module.no_heads_ipa,
                            'no_qk_points': self.structure_module.no_qk_points,
                            'no_v_points': self.structure_module.no_v_points,
                            'dropout_rate': self.structure_module.dropout_rate,
                            'no_blocks': self.structure_module.no_blocks,
                            'no_transition_layers': self.structure_module.no_transition_layers,
                            'no_resnet_blocks': self.structure_module.no_resnet_blocks,
                            'no_angles': self.structure_module.no_angles,
                            'trans_scale_factor': self.structure_module.trans_scale_factor,
                            'epsilon': self.structure_module.epsilon,
                            'inf': self.structure_module.inf,
                            'is_multimer': self.structure_module.is_multimer,
                            }
                        }
            
            # analyze_storage_size(save_data)

            print("Cloning all tensors to break shared storage links...")
            cloned_data_to_save = deep_clone(save_data)
            print("Cloning finished.")

            print("\nAnalyzing storage breakdown of CLONED data:")
            analyze_storage_size(cloned_data_to_save)

            print("Saving the cloned data...")
            torch.save(cloned_data_to_save, structure_path)
            print("Save complete.")

            # torch.save(save_data, structure_path)
    ...
    def forward(self, batch, model_dir):
        """
        Args:
            batch:
        ...
        if model_dir is not None:
            if "asym_id" in batch:
                save_data = {'num_recycles':torch.tensor(num_recycles, device=feats["aatype"].device),
                                'asym_id':outputs["asym_id"],
                                'heads_state':self.aux_heads.state_dict(),
                                'heads':self.config["heads"],
                                }
            else:
                save_data = {
                'num_recycles':torch.tensor(num_recycles, device=feats["aatype"].device),
                'heads_state':self.aux_heads.state_dict(),
                'heads':self.config["heads"],
                }

            head_path = model_dir+"head.pth"
            print('save head model data is true')
            print('head_path',head_path)
            torch.save(save_data, head_path)
