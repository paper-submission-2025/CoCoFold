...
def run_model(model, batch, tag, output_dir, model_dir):
    with torch.no_grad():
        # Temporarily disable templates if there aren't any in the batch
        if isinstance(model, torch.nn.DataParallel):
            template_enabled = model.module.config.template.enabled
            model.module.config.template.enabled = template_enabled and any([
                "template_" in k for k in batch
            ])

            logger.info(f"Running inference for {tag}...")
            t = time.perf_counter()
            out = model(batch, model_dir)
            inference_time = time.perf_counter() - t
            logger.info(f"Inference time: {inference_time}")
            update_timings({tag: {"inference": inference_time}}, os.path.join(output_dir, "timings.json"))

            model.module.config.template.enabled = template_enabled
        else:
